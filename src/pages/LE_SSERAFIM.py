...
# session state 선언
page_path = "pages/"
st.session_state['idol_group'] = "LE SSERAFIM"
...
def main():
    ...
    with st.container():
        ...
        freq_fig = create_frequency_chart(group_df[['Group', 'Title', 'date', 'predicted_label']],
                                          date_range=[min_date, max_date])
        sentiment_plot(freq_fig, key="frequency_chart")
        fig = create_sentiment_chart(group_df[['Group', 'Title', 'date', 'predicted_label']],
                                     date_range=[min_date, max_date])
    ...
    selected_member = carousel_component(data=member_thumbnail, layout='alternate',
                                         key="member_carousel") or default_selected_member
