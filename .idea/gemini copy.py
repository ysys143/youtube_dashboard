...
def generate_summary_for_article(llm, prompt_template, row, query):
    ...
    # 정규 표현식으로 "SUMMARY:"나 "SUMMARY :"를 제거
    summary = re.sub(r'SUMMARY\s*:', '', summary).strip()

    return f"{date}\n[{title}]\n{url}\n{summary}"

def stream_summarizer(query, articles):
    # LLM 준비
    #generation_config = {"온도": 0.7, "topP": 0.8, "topK": 40}
    generation_config = {"온도": 0.2, "topP": 0.8, "topK": 10}
...
#    # 각 기사별로 요약 생성 후 바로 반환
#    for _, row in articles.iterrows():
#        summary = generate_summary_for_article(llm, prompt_template, row, query)
#        yield summary +"\n\n"  # 요약문을 생성할 때마다 반환
    ...
    for _, row in articles.iterrows():
        summary = generate_summary_for_article(llm, prompt_template, row, query)
        summaries.append(summary)  # 딕셔너리 형태로 추가
...
def get_chatbot_response(group, date):
    ...
    try:
        ...
        # 요약 답변 생성
        summaries = stream_summarizer(query, articles)
